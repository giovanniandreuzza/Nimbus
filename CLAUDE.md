...
| `Builder` is nested in `Nimbus`, not in a companion object                          | Kotlin reaches a nested classifier as `Outer.Companion.Nested`, never `Outer.Nested`, so a companion that existed only to hold the builder forced every caller to write `Nimbus.Companion.Builder()` |
